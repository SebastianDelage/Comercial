...
export default function GalleryImageUploader({
  ...
  function handleUpload() {
    if (!images.length) {
      setError('Seleccioná al menos una fotografía.')
      return
    }
    ...
    startTransition(async () => {
      ...
      try {
        ...
        for (let index = 0; index < images.length; index++) {
          ...
          setProgressText(
            `Subiendo ${index + 1} de ${images.length}: ${image.file.name}`
          )
        ...
        setProgressText('Guardando fotografías...')
    ...
    <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-6">
      ...
      <div className="text-center">
        ...
        <p className="mt-2 text-sm text-gray-500">
          ...
          Formatos permitidos: JPG, PNG y WebP. Máximo 6 MB
          por archivo.
        </p>
        ...
        <button
          ...
          className="mt-5 rounded-xl bg-cyan-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-cyan-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
      ...
      {progressText && (
        <p className="mt-4 text-center text-sm font-medium text-cyan-700">
          {progressText}
        </p>
      )}
