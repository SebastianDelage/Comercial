...
function getAlbumFormData(formData: FormData) {
  ...
  const displayOrder = Number.isFinite(rawDisplayOrder)
    ? Math.max(0, Math.trunc(rawDisplayOrder))
    : 0
...
export async function createAlbum(formData: FormData) {
  ...
  const album = getAlbumFormData(formData)
  ...
  redirect(`/admin/gallery/${data.id}/images`)
}
...
export async function updateAlbum(
  ...
  const { data: currentAlbum, error: currentError } =
    await supabase
      .from('gallery_albums')
      .select('slug')
      .eq('id', id)
  ...
  revalidatePath('/admin/gallery')
  revalidatePath('/galeria')
  revalidatePath(`/galeria/${currentAlbum.slug}`)
...
export async function createGalleryImages(
  ...
  revalidatePath(`/admin/gallery/${albumId}/images`)
  revalidatePath('/admin/gallery')
  revalidatePath('/galeria')
  revalidatePath(`/galeria/${album.slug}`)
}
...
export async function deleteGalleryImage(imageId: string) {
  ...
  const albumRelation = Array.isArray(image.gallery_albums)
  ...
  if (albumRelation?.slug) {
    revalidatePath(`/galeria/${albumRelation.slug}`)
  }
}
export async function deleteAlbum(
  ...
  if (albumError || !album) {
    ...
    return {
      error: 'No se encontró el álbum que intentás eliminar.',
    }
  }

  const imagePaths = (album.gallery_images ?? [])
   ...
   * gallery_images se elimina mediante ON DELETE CASCADE.
   */
  const { error: deleteError } = await supabase
    .from('gallery_albums')
    .delete()
  ...
  if (deleteError) {
    console.error('Error eliminando álbum:', deleteError)

    return {
      error:
        'No se pudo eliminar el álbum de la base de datos.',
    }
  }

  /*
   * Después intentamos limpiar Storage.
   * No impedimos que la eliminación quede completada si falla
   * únicamente la limpieza de algún archivo.
   */
  if (storagePaths.length > 0) {
    ...
    if (storageError) {
      console.error(
        'El álbum fue eliminado, pero falló la limpieza de Storage:',
        storageError
      )
    }
  }
